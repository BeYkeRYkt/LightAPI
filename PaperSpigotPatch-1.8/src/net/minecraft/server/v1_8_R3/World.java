...
public abstract class World implements IBlockAccess {
	...
	public boolean setTypeAndData(BlockPosition blockposition, IBlockData iblockdata, int i) {
		...
		if (!this.isValidLocation(blockposition)) {
			...
			if (iblockdata1 == null) {
				...
				/*
				 * if ((i & 2) != 0 && (!this.isClientSide || (i & 4) == 0) && chunk.isReady()) { this.notify(blockposition); }
				 * 
				 * if (!this.isClientSide && (i & 1) != 0) { this.update(blockposition, iblockdata1.getBlock()); if (block.isComplexRedstone()) { this.updateAdjacentComparators(blockposition, block); } }
				 */
	...
	public void tickEntities() {
		...
		this.M = false;
		/*
		 * CraftBukkit start - Moved up if (!this.c.isEmpty()) { this.tileEntityList.removeAll(this.c); this.h.removeAll(this.c); this.c.clear(); } // CraftBukkit end
		 */
		...
		if (!this.b.isEmpty()) {
			for (int l = 0; l < this.b.size(); ++l) {
				...
				if (!tileentity1.x()) {
					/*
					 * CraftBukkit start - Order matters, moved down if (!this.h.contains(tileentity1)) { this.a(tileentity1); } // CraftBukkit end
					 */
	...
	public boolean updateLight(EnumSkyBlock enumskyblock, BlockPosition position) {
		return c(enumskyblock, position);
	}
}
